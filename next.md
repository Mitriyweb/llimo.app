**Кроки (`steps`)** – це масив, що містить результати кожного окремого етапу генерації.  
Кожен елемент `StepResult` включає:

| Поле | Опис |
|------|------|
| `content` | Частини контенту, згенеровані в цьому кроці (текст, reasoning, файли, джерела, виклики інструментів тощо). |
| `text` | Текст, отриманий у цьому кроці. |
| `reasoning` / `reasoningText` | Reasoning‑частини та їх текст, якщо модель їх генерує. |
| `files` | Файли, створені під час кроку. |
| `sources` | Джерела (наприклад, URL‑посилання), використані для створення відповіді. |
| `toolCalls` / `toolResults` | Відображають виклики інструментів та їх результати. |
| `finishReason` | Чому генерація завершилася в цьому кроці (наприклад, `stop`, `tool-calls` тощо). |
| `usage` | Метрика токенів (вхідних, вихідних, загальних, reasoning, cached). |
| `warnings` | Попередження від провайдера. |
| `request` / `response` | Метадані HTTP‑запиту та відповіді. |
| `providerMetadata` | Додаткова інформація від провайдера. |

Кожен новий крок успадковує контекст попереднього, тому `steps` дозволяє аналізувати не лише фінальний результат, а й всі проміжні стані процесу (наприклад, коли модель спочатку викликає інструмент, потім отримує його результат і продовжує генерувати текст).

---

**`onStepFinish`** – це колбек, який викликається **після завершення кожного кроку**.  

```ts
onStepFinish?: (result: OnStepFinishResult) => Promise<void> | void
```

`OnStepFinishResult` містить ті ж поля, що і `StepResult`, плюс:

- `isContinued` – `true`, якщо після цього кроку заплановано продовження (очікується наступний крок).  
- `providerMetadata` – метадані провайдера, доступні лише після завершення кроку.

Колбек дозволяє:

1. **Логувати** або **зберігати** проміжні дані (наприклад, записувати `toolCalls` у базу).  
2. **Модифікувати** наступний крок: хоча сам колбек не змінює стан, його результат можна використати в `prepareStep` для підготовки наступного кроку.  
3. **Виконувати побічні ефекти** (надсилати сповіщення, оновлювати UI, збирати статистику).  
4. **Зупиняти подальшу генерацію**: виводячи помилку або повертаючи спеціальний результат, можна перервати процес.

Таким чином, `steps` дає повний журнал процесу, а `onStepFinish` забезпечує точку втручання між кроками, дозволяючи підлаштовувати поведінку або збирати додаткову інформацію під час роботи моделі.
